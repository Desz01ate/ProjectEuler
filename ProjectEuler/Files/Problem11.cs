...
namespace ProjectEuler.Files
{
    ...
    {
        public readonly int[][] Grid = new int[][]{
        ...
        public object Solve()
        {
            ...
            for (var y = 0; y < Grid.Length; y++)
            {
                ...
                {
                    max = Math.Max(max, Calculate(x, y, 1, 0, countElement));
                    max = Math.Max(max, Calculate(x, y, 0, 1, countElement));
                    max = Math.Max(max, Calculate(x, y, 1, 1, countElement));
                    max = Math.Max(max, Calculate(x, y, 1, -1, countElement));
                }
            }
            return max;
        ...
        {
            //is the indexer still in Grid boundary?
            return (0 <= y && y < Grid.Length) && (0 <= x && x < Grid[y].Length);
        }
        private int Calculate(int x, int y, int dx, int dy, int z)
        {
            if (!IsInBoundary(x + (z - 1) * dx, y + (z - 1) * dy)) return -1;
            var result = 1;
            for (var i = 0; i < z; i++, x += dx, y += dy)
            {
                result *= Grid[y][x];
            }
            return result;
